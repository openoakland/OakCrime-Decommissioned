...
def normPLogFName(frootOrig):
    ...
    if date_pat.match(froot) is None:
            ...
            if fld == 'loss':
                if v == 'none' or v == "n/a" or v == 'unk' or v == 'unknown' or v == 'no loss':
                    ...
                    for loss in lossList:
                        loss = loss.replace('.', '').strip()
                        if len(loss) == 0:
                            continue
                        if loss.find('cash') != -1 or loss.find('currency') != -1 or loss.find('money') != -1 \
                                or loss.find('$') != -1:
                            rll.append('cash')
                        elif loss.find('phone') != -1:
                            rll.append('phone')
                        elif loss.find('personal') != -1:
                            rll.append('personalProperty')
                        elif loss.find('gun') != -1 or loss.find('firearm') != -1 or loss.find('pistol') != -1:
                            rll.append('gun')
                        else:
                            rll.append(loss)
...
def addGeoCode2(dlogTbl, gconn, verbose=None):
    ...
    for i, dlogCID in enumerate(allCID):
        ...
        if oakFnd:
            ...
            newDLogTbl[dlogCID] = newdlog
            if verbose == 1:
                print(('addGeoCode G:  %d %s "%s" "%s" %f %f' % (
                    i, dlogCID, loc, loc2, newdlog['XLng'], newdlog['YLat'])))
            nhit += 1
        else:

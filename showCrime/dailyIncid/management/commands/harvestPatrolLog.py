...
def filterNewDLog(newIncid):
    ...
    print('filterNewDLogTbl: NIn=%d NDup=%d NNew=%d/%d' % (len(newIncid), ndup, nnew, len(newDLog)))
    ...
    for yrf in yearFolders:
        ...
        for monf in yrf.get_items(limit=100):
            ...
            for dayf in monf.get_items(limit=100):
                if dayf.type != 'file':
                    print('getBoxIDs: skipping day non-file %s / %s %s %s' % (mkey, dayf.id, dayf.type, dayf.name))
                    nskip += 1
                    continue
    ...
    for yrf in yearFolders:
        ...
        for monf in yrf.get_items(limit=100):
            ...
            for dayf in monf.get_items(limit=100):
                ...
                if prevDayModDT is not None and not (dayModDT > prevDayModDT and dayModDT > lastUpdate):
                    continue
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        matchLogStream.write('dRptNo,dLoc,dxlng,dylat,dDT,dPC,iCID,iAddr,ixlng,iylat,iDT,iCC,iCType,iDesc,matchScore,'
                             'idDist,distMeter,dayDiff,majorCrime\n')
        ...
        for i, cid in enumerate(allCID):
            ...
            try:
                ...
                assert bool(newOC.idx), 'harvestDLog: No idx for update?! %s\n%s' % (cid, pprintOC(newOC))
                newOC.save()
                nupdate += 1
            except Exception as e:
        ...
        for i, cid in enumerate(allCID):
            ...
            try:
                assert not bool(newOC.idx), 'harvestPatrolLog: Idx for insert?! %s\n%s' % (cid, pprintOC(newOC))
                newOC.save()
                nupdate += 1
            except Exception as e:

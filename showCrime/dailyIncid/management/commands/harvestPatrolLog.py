"""
Capture new OPD patrol logs from box.com
...
from dailyIncid.management.commands import parsePatrolLog as parsePL, postPatrolLog as postPL
from dailyIncid.models import *

HARVEST_ROOT_DIR = os.path.join(settings.MEDIA_ROOT, 'PLHarvest')
...
def getMiss(missInfo):
    ...
    kbits.insert(0, HARVEST_ROOT_DIR)
...
def create_box_client():
    auth = boxsdk.JWTAuth(
        client_id=settings.BOX_CLIENT_ID,
        client_secret=settings.BOX_CLIENT_SECRET,
        enterprise_id=settings.BOX_ENTERPRISE_ID,
        jwt_key_id=settings.BOX_JWT_KEY_ID,
        rsa_private_key_file_sys_path=settings.BOX_RSA_FILE_PATH,
        rsa_private_key_passphrase=settings.BOX_RSA_FILE_PASSPHRASE
    )
...
def makeBoxConnection():
    ...
    CurrBoxClient = create_box_client()
...
def makeGConnection():
    global CurrGClient
    CurrGClient = googlemaps.Client(key=settings.GOOGLE_MAPS_API_KEY)
...
def compBox2Dir(boxIDTbl):
    ...
    print(
        'compBox2Dir: NEntries=%d NFnd=%d NMiss=%d NDirMismatch=%d' % (len(boxIDTbl), nfnd, len(missing), ndirMismatch))
    return missing
...
def getBoxIDs(verbose=True):
    ...
    for yrf in yearFolders:
        ...
        if ykey in boxIDTbl:
            ...
            yrf.id, yrf.type, ykey, boxIDTbl[ykey]['id']))
            nskip += 1
            continue
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        boxIDFile = os.path(HARVEST_ROOT_DIR, 'boxIDTbl.json')
        ...
        logData = parsePL.collectDailyLogs(files2parse, HARVEST_ROOT_DIR)

        if checkPoint:
            # cache results of potentially LONG PDF parse
            ldf = os.path.jpoin(HARVEST_ROOT_DIR, f'logData_{dateStr}.json')
        ...
        if checkPoint:
            geof = os.path.join(HARVEST_ROOT_DIR, f'geoIncid_{dateStr}.pkl')
        ...
        matchLogFile = os.path.join(HARVEST_ROOT_DIR, f'matchLog_{dateStr}.csv')
        matchLogStream = open(matchLogFile, 'w')
        matchLogStream.write(
        ...
        if checkPoint:
            dlf = os.path.join(HARVEST_ROOT_DIR, f'dlMatch_{dateStr}.pkl')
            with open(dlf, 'wb') as f:
                pickle.dump(dlMatchTbl, f, pickle.HIGHEST_PROTOCOL)

            unf = os.path.join(HARVEST_ROOT_DIR, f'unMatch_{dateStr}.pkl')

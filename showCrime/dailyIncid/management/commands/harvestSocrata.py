...
> crontab -e
0 1 * * * * source python /pathToApplicationDir/.../manage.py runcrons HarvestSocrataJob >
    /pathTologFiles/.../cronjob.log
...
def mergeIncid2List(matchObjList, newOC):
    ...
    if minNUpdate == int(1e6) or minNUpdate == 0:
        ...
        try:
            ...
            print('mergeIncid2List: cant save?! cid=%s bestIdx=%s' % (newOC.opd_rd, bestMatch.idx))
            return None
        return bestMatch
...
def mergeIncidSingle(prevObj, newOC):
    ...
    if len(updates) > 0:
        ...
        try:
            ...
            print('mergeIncidSingle: cant save?! cid=%s prevIdx=%s' % (newOC.opd_rd, prevObj.idx))
            return None
...
def mergeList(results, startDate, verboseFreq=None, rptAll=False):
    ...
    for incidIdx, socDict in enumerate(results):
        ...
        if verboseFreq is not None and incidIdx % verboseFreq == 0:

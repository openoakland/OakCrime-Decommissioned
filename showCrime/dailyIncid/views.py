...
# TODO Use the real decorator: https://docs.djangoproject.com/en/1.11/topics/auth/default/#the-login-required-decorator
def rikNoLogin(cbfn):
    return cbfn
...
# TODO Replace with https://stackoverflow.com/a/5201357
def index(request):
...
def heatmap(request, mapType='general'):
    ...
    context = {
        ...
        'cylat': FTVL_lat,
        'heatmap': True,
        'minDate': [minDate.year, minDate.month - 1, minDate.day],
        'maxDate': [nowDT.year, nowDT.month - 1, nowDT.day],
        'minSlider': [minDate.year, minDate.month - 1, minDate.day],
        'maxSlider': [nowDT.year, nowDT.month - 1, nowDT.day],
    }
...
def hybridQual(request, mapType):
    ...
    minSelectDate = timezone.make_aware(datetime.strptime(minSelectDateStr, selectDateFmt))
    maxSelectDate = timezone.make_aware(datetime.strptime(maxSelectDateStr, selectDateFmt))
...
def downloadNCPC(request, beat, minDateDigit, maxDateDigit):
    ...
    minDate = timezone.make_aware(datetime.strptime(minDateDigit, '%y%m%d'))
    maxDate = timezone.make_aware(datetime.strptime(maxDateDigit, '%y%m%d'))
...
def docView(request):
    ...
    return render(request, 'dailyIncid/doc.html', context)


class BeatAPI(generics.ListAPIView):
    """API view for crimes from specified beat restricted to last two years """
...
class NearHereAPI(generics.ListAPIView):
    """API view for crimes within 500m of this longitude_latitude, restricted to last 6 months """
...
class CrimeCatAPI(generics.ListAPIView):
    """API view for crimes of this crime category restricted to last 6 months """
    serializer_class = serializers.IncidSerializer
